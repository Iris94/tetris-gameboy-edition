...
function getDataNinjaStrike() {
    ...
    animationAccess = ninjaCells.length > 0;
    return { ninjaCells, ninjaIds, singularCellsToClear, animationAccess };
}

export async function ninjaStrike() {
    const { ninjaCells, ninjaIds, singularCellsToClear, animationAccess } = getDataNinjaStrike();
    if (!animationAccess) return 0; 
